(function(factory){
    ...
    function updateEvent(){
        ...
        $(document).on('click', 'button.btn-answer-okay', function(){
            ...
            $Request.postInfo($Request.getRouter('answerInfo'), {id : _id, answer : _content}, function(data){
                if('object' === typeof data && 'undefined' !== typeof data['resCode'] && 1 == parseInt(data['resCode'])){
                    ...
                    setTimeout(function(){
                        _message.hideIt();
                        $("#rightListInner ul.resource-list li.active").find('.status').html('已答');
        ...
        $(document).on('click', 'button#updateChess', function(){
             var _id = parseInt($(this).data('id')),
                 ...
                 _activeBtn =  $("#rightListInner ul.resource-list li.active .btn-res-item");
             //提交作业
             $Request.postInfo($Request.getRouter('answerInfo'), {id : _id, answer : _answer}, function(data){
                if('object' === typeof data && 'undefined' !== typeof data['resCode'] && 1 == parseInt(data['resCode'])){
                    _activeBtn.data('answer',_answer);
                    _message.setContent(template('tpl-message', {
                        message : '保存成功'
                    })).showIt();
                    $("#updateChess").hide();
                    ...
                    $("#rightListInner ul.resource-list li.active button.btn-res-item").data('answer',_answer);
                    $("#rightListInner ul.resource-list li.active button.btn-res-item").data('status',2);
                    setTimeout(function(){
                        _message.hideIt();
                        $("#rightListInner ul.resource-list li.active").find('.status').html('已答');
        ...
        //订正
        $(document).on('click', 'button.btn-redo-homework', function(){
            alert("订正题目");
        })
    }
    ...
    function resourceEvent(){
        ...
        $(document).on('focus', 'textarea#inputAnswer', function(){
            $(this).removeClass('empty');
        });
    }
