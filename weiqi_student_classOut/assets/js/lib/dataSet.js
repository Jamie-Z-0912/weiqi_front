...
class KiFu{
    ...
    removeProp(_pos, _name){
        ...
        // revised by yk
        var _chessList = this.chessList;
        if(_pos.c){
            var clickPos = _pos.pos;
            var repeatIndex;
            for( var i in _chessList){
                var chessListPos = _chessList[i].move.pos;
                if (clickPos == chessListPos){
                    repeatIndex = i;
                }
            }
            if (repeatIndex) this.chessList.splice(repeatIndex,1);
        }
    }
...
class DataSet extends KiFu{
     ...
     */
    setAEChess(_chess){
        if('object' === typeof _chess) super.removeProp(_chess, PropsName.AE);
    }
